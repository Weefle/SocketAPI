						...
						if (read == null)
							...
							if (Security.level >= 2 && Security.Target.RSA == null) { // Is RSA enabled? Do we received
								...
								if (!read.equals("--end--"))
									...
									Security.Target.RSA = Utils.RSA.loadPublicKey(RSA_key); // Convert it to a PublicKey
																							// object
									// Now we send our AES key encrypted with RSA
									IO.writer.println(Utils.RSA.encrypt(Utils.AES.toString(Security.Self.AES),
											Security.Target.RSA));
									IO.writer.println("--end--");
									IO.writer.flush();
								}
								...
								if (!read.equals("--end--"))
									...
									if (Security.level == 2) {
										Security.Target.AES = Utils.AES
												.toKey(Utils.RSA.decrypt(AES_key, Security.Self.RSA.getPrivate()));
									}
									writeJSON("SocketAPI", "handshake");
								}
							} else {
								...
								if (Security.level == 0)
									decrypted = read;
								if (Security.level >= 1)
									decrypted = Utils.AES.decrypt(read, Security.Self.AES);
